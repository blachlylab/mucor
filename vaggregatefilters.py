# vaggregatefilters.py
#

class VaggregateFilters(object):
	"""docstring for VaggregateFilters"""
	def __init__(self):
		super(VaggregateFilters, self).__init__()
		self._filterSets = None					# internally use dict
		self._filters = None					# internally use dict

	def loadFromXML(self, rootNode):
		'''Load filters and filterSets from vaggregate XML config file.
		Overwrites any existing filters or filterSets stored in this object.'''

		# namespace
		ns = "{urn:osuccc:hcrg:vaggregate}"

		if not isinstance(rootNode, ET.Element):
			raise TypeError
		if root.tag != ns + "vaggregate":
			raise ValueError
